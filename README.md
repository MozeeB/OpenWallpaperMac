...
| **Rotation** | Command-click several wallpapers and rotate them on a preset interval (5 s to 1 h) or a custom one (5 s to 24 h), in order or shuffled; switches cross-fade and pause with playback |
| **Per display and per Space** | Every display, and every Space (Desktop 1, Desktop 2, ...), can have its own wallpaper or rotation; Spaces without one use the display default ([ADR 0004](docs/adr/0004-per-space-wallpapers.md)) |
